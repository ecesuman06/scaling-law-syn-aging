...
%}
%%
function[h,L,MX,MED,bw]=violin2(Y,shift,varargin)
%defaults:_____________________
xL=[];
fc=[1 0.5 0];
lc='k';
alp=0.5;
...
x = [];
lw=1;
%_____________________convert single columns to cells:
if iscell(Y)==0
    Y = num2cell(Y,1);
end
...
if isempty(find(strcmp(varargin,'linew')))==0
    lw = varargin{find(strcmp(varargin,'linew'))+1};
end
...
for i=i:size(Y,2)
    if isempty(lc) == 1
        if setX == 0
            fill([F(:,i)+i;flipud(i-F(:,i))]+shift,[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor','none','LineWidth',lw);
        else
            fill([F(:,i)+x(i);flipud(x(i)-F(:,i))]+shift,[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor','none','LineWidth',lw);
        end
    else
        if setX == 0
            fill([F(:,i)+i;flipud(i-F(:,i))]+shift,[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor',lc,'LineWidth',lw);
        else
            fill([F(:,i)+x(i);flipud(x(i)-F(:,i))]+shift,[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor',lc,'LineWidth',lw);
        end
    end
    hold on
    if setX == 0
        if plotmean == 1
            p(1)=plot([interp1(U(:,i),F(:,i)+i,MX(:,i)), interp1(flipud(U(:,i)),flipud(i-F(:,i)),MX(:,i)) ]+shift,[MX(:,i) MX(:,i)],mc,'LineWidth',2);
        end
        if plotmedian == 1
            p(2)=plot([interp1(U(:,i),F(:,i)+i,MED(:,i)), interp1(flipud(U(:,i)),flipud(i-F(:,i)),MED(:,i)) ]+shift,[MED(:,i) MED(:,i)],medc,'LineWidth',2);
        end
    elseif setX == 1
        if plotmean == 1
            p(1)=plot([interp1(U(:,i),F(:,i)+i,MX(:,i))+x(i)-i, interp1(flipud(U(:,i)),flipud(i-F(:,i)),MX(:,i))+x(i)-i]+shift,[MX(:,i) MX(:,i)],mc,'LineWidth',2);
        end
        if plotmedian == 1
            p(2)=plot([interp1(U(:,i),F(:,i)+i,MED(:,i))+x(i)-i, interp1(flipud(U(:,i)),flipud(i-F(:,i)),MED(:,i))+x(i)-i]+shift,[MED(:,i) MED(:,i)],medc,'LineWidth',2);
        end
    end
end
